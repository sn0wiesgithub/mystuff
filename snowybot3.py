...
class BotEngine(QMainWindow): 
    def __init__(self):
        ...
        self.log("System initialized. Loading Dice...")
    ...
    def inject_login(self):
        ...
        QTimer.singleShot(25000, self.check_ready)
    ...
    def check_balance(self):
        js = "document.getElementById('pct_balance').value;"
    ...
    def process_tick(self, bal_str):
        ...
        try:
            current_real = Decimal(str(bal_str))
        except Exception as e:
            self.log(f"Balance parse error: {e}")
            return
        ...
        if current_real!=self.shadow:
            ...
            if ((delta != self.cat) and (delta != (0-self.cat)) and (delta != 0)):
                self.log(f"🚨 SECURITY: Delta {delta:+.8f} > Bet {self.cat}")
                self.heartbeat = False
            ...
            if self.tracked_balance>=(self.orgy+(self.tens*self.fart)):
                ...
                self.uppers = Decimal("6.9")
                self.downers = Decimal("2.9")
                self.felix = Decimal(float(round(self.mighty, 8)))
                self.orgy = Decimal(float(round(self.mighty, 8)))
            if ((self.tracked_balance > (self.felix + (self.cat*self.uppers))) and (self.tracked_balance < (self.felix + (self.cat*(self.uppers+(Decimal("1.0"))))))):
                 ...
                 self.felix = Decimal(float(round(self.tracked_balance, 8)))
            if ((self.tracked_balance < (self.felix - (self.cat*self.downers))) and (self.tracked_balance > (self.felix - (self.cat*(self.downers+(Decimal("1.0"))))))):
                 ...
                 self.downers = Decimal("4.9")
                 self.felix = Decimal(float(round(self.tracked_balance, 8)))
            if ((self.tracked_balance<(self.felix-(self.cat*(self.downers+(Decimal("1.0")))))) or (self.tracked_balance >= (self.felix + (self.cat*(Decimal("10.0"))))) or ((self.cat>self.tabby) and (self.tracked_balance < (self.orgy + (self.tens * self.fart))) and (self.tracked_balance > (self.felix + (self.cat*(self.uppers+(Decimal("1.0")))))))):           
